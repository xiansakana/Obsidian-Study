...
![1653984583289](https://cdn.jsdelivr.net/npm/redis-itheima/1653984583289.png)

那么Redis将在底层创建两个SDS，其中一个是包含“name”的SDS，另一个是包含“虎哥”的SDS。

Redis是C语言实现的，其中SDS是一个结构体，源码如下：

![1653984624671](https://cdn.jsdelivr.net/npm/redis-itheima/1653984624671.png)

例如，一个包含字符串“name”的sds结构如下：

![1653984648404](https://cdn.jsdelivr.net/npm/redis-itheima/1653984648404.png)

SDS之所以叫做动态字符串，是因为它具备动态扩容的能力，例如一个内容为“hi”的SDS：

![1653984787383](https://cdn.jsdelivr.net/npm/redis-itheima/1653984787383.png)
...
![1653984923322](https://cdn.jsdelivr.net/npm/redis-itheima/1653984923322.png)

其中的encoding包含三种模式，表示存储的整数大小不同：

![1653984942385](https://cdn.jsdelivr.net/npm/redis-itheima/1653984942385.png)

为了方便查找，Redis会将intset中所有的整数按照升序依次保存在contents数组中，结构如图：

![1653985149557](https://cdn.jsdelivr.net/npm/redis-itheima/1653985149557.png)
...
![1653985197214](https://cdn.jsdelivr.net/npm/redis-itheima/1653985197214.png)
...
![1653985276621](https://cdn.jsdelivr.net/npm/redis-itheima/1653985276621.png)

源码如下：

![1653985304075](https://cdn.jsdelivr.net/npm/redis-itheima/1653985304075.png)

![1653985327653](https://cdn.jsdelivr.net/npm/redis-itheima/1653985327653.png)
...
![1653985396560](https://cdn.jsdelivr.net/npm/redis-itheima/1653985396560.png)

当我们向Dict添加键值对时，Redis首先根据key计算出hash值（h），然后利用 h & sizemask来计算元素应该存储到数组中的哪个索引位置。我们存储k1=v1，假设k1的哈希值h =1，则1&3 =1，因此k1=v1要存储到数组角标1位置。

![1653985497735](https://cdn.jsdelivr.net/npm/redis-itheima/1653985497735.png)

Dict由三部分组成，分别是：哈希表（DictHashTable）、哈希节点（DictEntry）、字典（Dict）

![1653985570612](https://cdn.jsdelivr.net/npm/redis-itheima/1653985570612.png)

![1653985586543](https://cdn.jsdelivr.net/npm/redis-itheima/1653985586543.png)

![1653985640422](https://cdn.jsdelivr.net/npm/redis-itheima/1653985640422.png)
...
![1653985716275](https://cdn.jsdelivr.net/npm/redis-itheima/1653985716275.png)

![1653985743412](https://cdn.jsdelivr.net/npm/redis-itheima/1653985743412.png)
...
![1653985824540](https://cdn.jsdelivr.net/npm/redis-itheima/1653985824540.png)
...
![1653985987327](https://cdn.jsdelivr.net/npm/redis-itheima/1653985987327.png)
...
![1653986328124](https://cdn.jsdelivr.net/npm/redis-itheima/1653986328124.png)
...
![1653986474927](https://cdn.jsdelivr.net/npm/redis-itheima/1653986474927.png)
...
![1653986642777](https://cdn.jsdelivr.net/npm/redis-itheima/1653986642777.png)

以下是QuickList的和QuickListNode的结构源码：

![1653986667228](https://cdn.jsdelivr.net/npm/redis-itheima/1653986667228.png)
...
![1653986771309](https://cdn.jsdelivr.net/npm/redis-itheima/1653986771309.png)

SkipList（跳表）首先是链表，但与传统链表相比有几点差异：
元素按照升序排列存储
节点可能包含多个指针，指针跨度不同。

![1653986813240](https://cdn.jsdelivr.net/npm/redis-itheima/1653986813240.png)

SkipList（跳表）首先是链表，但与传统链表相比有几点差异：
元素按照升序排列存储
节点可能包含多个指针，指针跨度不同。

![1653986877620](https://cdn.jsdelivr.net/npm/redis-itheima/1653986877620.png)
...
![1653986956618](https://cdn.jsdelivr.net/npm/redis-itheima/1653986956618.png)
...
![1653987103450](https://cdn.jsdelivr.net/npm/redis-itheima/1653987103450.png)

如果存储的字符串是整数值，并且大小在LONG_MAX范围内，则会采用INT编码：直接将数据保存在RedisObject的ptr指针位置（刚好8字节），不再需要SDS了。

![1653987159575](https://cdn.jsdelivr.net/npm/redis-itheima/1653987159575.png)

![1653987172764](https://cdn.jsdelivr.net/npm/redis-itheima/1653987172764.png)

![1653987202522](https://cdn.jsdelivr.net/npm/redis-itheima/1653987202522.png)
...
![1653987240622](https://cdn.jsdelivr.net/npm/redis-itheima/1653987240622.png)
...
![1653987313461](https://cdn.jsdelivr.net/npm/redis-itheima/1653987313461.png)
...
![1653987342550](https://cdn.jsdelivr.net/npm/redis-itheima/1653987342550.png)
...
![1653987388177](https://cdn.jsdelivr.net/npm/redis-itheima/1653987388177.png)
...
![1653992091967](https://cdn.jsdelivr.net/npm/redis-itheima/1653992091967.png)
...
![1653992121692](https://cdn.jsdelivr.net/npm/redis-itheima/1653992121692.png)

![1653992172526](https://cdn.jsdelivr.net/npm/redis-itheima/1653992172526.png)
...
![1653992238097](https://cdn.jsdelivr.net/npm/redis-itheima/1653992238097.png)
...
![1653992339937](https://cdn.jsdelivr.net/npm/redis-itheima/1653992339937.png)

zset集合如下：

![1653992360355](https://cdn.jsdelivr.net/npm/redis-itheima/1653992360355.png)
...
![1653844970346](https://cdn.jsdelivr.net/npm/redis-itheima/1653844970346.png)

用户的应用，比如redis，mysql等其实是没有办法去执行访问我们操作系统的硬件的，所以我们可以通过发行版的这个壳子去访问内核，再通过内核去访问计算机硬件

![1653845147190](https://cdn.jsdelivr.net/npm/redis-itheima/1653845147190.png)
...
![1653896377259](https://cdn.jsdelivr.net/npm/redis-itheima/1653896377259.png)
...
![1653896687354](https://cdn.jsdelivr.net/npm/redis-itheima/1653896687354.png)
...
![1653897115346](https://cdn.jsdelivr.net/npm/redis-itheima/1653897115346.png)
...
![1653900022580](https://cdn.jsdelivr.net/npm/redis-itheima/1653900022580.png)
...
![1653900721427](https://cdn.jsdelivr.net/npm/redis-itheima/1653900721427.png)

### 2.7 IO多路复用模型-epoll函数
...
![1653911776583](https://cdn.jsdelivr.net/npm/redis-itheima/1653911776583.png)
...
![1653911877542](https://cdn.jsdelivr.net/npm/redis-itheima/1653911877542.png)
...
![1653983341150](https://cdn.jsdelivr.net/npm/redis-itheima/1653983341150.png)
...
![1653983366243](https://cdn.jsdelivr.net/npm/redis-itheima/1653983366243.png)
...
![1653983652865](https://cdn.jsdelivr.net/npm/redis-itheima/1653983652865.png)
...
![1653984029506](https://cdn.jsdelivr.net/npm/redis-itheima/1653984029506.png)
...
![1653984085095](https://cdn.jsdelivr.net/npm/redis-itheima/1653984085095.png)
